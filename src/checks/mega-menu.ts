...
export const megaMenuCheck: Check = {
  ...
  async run(context: CheckContext): Promise<Issue[]> {
    ...
    // Wrap entire check in cleanup wrapper to ensure modals/menus are closed
    return withCleanup(page, async () => {
      const issues: Issue[] = [];

      try {
        logger.info(
          `Checking mega menu keyboard navigation on ${target.label}`
        );
        ...
        if (!hasLinkAsButtonIssue) {
          ...
          if (linkAsButtonItems.count > 0) {
            ...
            issues.push({
              ...
              title:
                "Navigation Dropdown Triggers Use Links Instead of Buttons",
        ...
        if (!hasMissingFocusIndicator) {
          ...
          if (missingFocusIndicators > 0) {
            ...
            const rawData: { screenshotBuffer?: Buffer; codeSnippet?: string } =
              {};
            try {
              await dismissPopupsWithGuards(page, [], {
                label: "mega-menu-focus-indicator",
              });
              await page.waitForTimeout(300);

              // Remove any residual search overlays that might obscure navigation
              await page.evaluate(() => {
            ...
            issues.push({
              ...
              wcagCriteria: ["2.4.7"],
              path: target.url,
              screenshot: rawData.screenshotBuffer
                ? "screenshot.png"
                : undefined,
              codeSnippet:
        ...
        for (let i = 0; i < itemsToTest.length; i++) {
          ...
          try {
            ...
            if (expandedAfterEnter !== "true") {
              ...
              const expandedAfterSpace = await item.getAttribute(
                "aria-expanded"
              );
...
WCAG Success Criteria: 2.1.1 Keyboard (Level A), 2.1.2 No Keyboard Trap (Level A)`,
            ...
            const expandedAfterEscape = await item.getAttribute(
              "aria-expanded"
            );
...
WCAG Success Criterion: 2.5.5 Target Size (Level AAA) - relates to motor control`,
        ...
        if (missingAltImages.count > 0) {
          ...
          try {
            ...
            if (annotationResult) {
              altTextRawData.screenshotBuffer =
