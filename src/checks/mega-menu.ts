...
WCAG Success Criterion: 1.4.3 Contrast (Minimum) (Level AA)`,
      ...
      if (smallTargets.count > 0) {
        ...
        issues.push({
          ...
          description: `${smallTargets.count} navigation links or buttons are smaller than 44×44px, making them difficult to tap accurately on mobile devices. This frustrates mobile users and reduces conversions.`,
          severity: "moderate",
          impact: "conversion",
          effort: "low",
          wcagCriteria: ["2.5.5"],
...
${
  smallTargets.count
} navigation elements are smaller than 44×44px (mobile usability issue).

Requirements:
1. Minimum touch target: 44×44px (WCAG AAA)
2. Recommended: 48×48px (Material Design)
3. Maintain spacing between targets
...
nav a,
nav button {
  padding: 12px 16px; /* Adjust to reach 44px height */
  min-height: 44px;
}

Method 2: Increase font size + padding
nav a {
  font-size: 16px;
...
Method 4: Icon buttons
.icon-button {
  width: 48px;
  height: 48px;
  padding: 12px;
  /* Icon itself can be 24px, button is 48px */
}

Common issues:
❌ Tiny "X" close buttons (< 30px)
❌ Small icon-only links (< 40px)
❌ Compact nav links on mobile (< 35px tall)

Spacing considerations:
/* Ensure targets don't overlap touch areas */
nav a {
  margin: 4px; /* 8px total gap between targets */
}

Mobile-specific:
@media (max-width: 768px) {
  nav a {
    padding: 14px 20px; /* Larger on mobile */
    min-height: 48px;
  }
}
...
2. Zoom to 200% and test tappability
3. Try on actual mobile device
4. Check spacing between adjacent links

Examples found:
${JSON.stringify(smallTargets.examples, null, 2)}
